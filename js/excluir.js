        ...
        .then((querySnapshot) => {
            querySnapshot.forEach((doc) => {
                db.collection("agendamentos").doc(doc.id).delete().then(() => {
                    ...
                    window.location.href = cordova.file.applicationDirectory + "consulta.html";
                }).catch((error) => {
                    console.error("Erro em remover documento: ", error);
                });
            });
        })
        .catch((error) => {
            console.log("Erro em remover documento: ", error);
        });
        }
        }
app.initialize();